...
so that I can benefit from the latest API changes and consolidate maintenance efforts.
```
...
### Phase 1 Details
The following describes our proposal to replace require_chef_omnibus with the new options `product_version` and `install_strategy` to replace `require_chef_omnibus`. It outlines requirements to maintain feature parity with Kitchen's current configuration options. Since `require_chef_omnibus` is a complex option it warranted this level of detail.
